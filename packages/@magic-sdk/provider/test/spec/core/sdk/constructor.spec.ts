...
import { MAGIC_GAS_API_URL, MAGIC_NFT_API_URL, MAGIC_RELAYER_FULL_URL, TEST_API_KEY } from '../../../constants';
...
test('Initialize `MagicSDK` with custom gasEndpoint', () => {
  const Ctor = createMagicSDKCtor();
  const magic = new Ctor(TEST_API_KEY, { gasEndpoint: 'https://example.com' });

  expect(magic.apiKey).toBe(TEST_API_KEY);
  assertEncodedQueryParams(magic.parameters, { gasEndpoint: 'https://example.com' });
  assertModuleInstanceTypes(magic);
});
